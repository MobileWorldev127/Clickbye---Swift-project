...
protocol AcceptDepartDates {
    func acceptData(_ data: Dates)
}

class DepartView: UIViewController, MDCalendarDelegate {
    
    var delegate: AcceptDepartDates?
    var dateСhosen: (()->())?
    ...
    //MARK: Actions
    @IBOutlet weak var calendarView: MDDatePicker! {
        didSet {
    ...
    @IBAction func cancelDepart(_ sender: UIButton) {
        removeAnimate()
    }
    ...
    func calendar(_ calendar: MDCalendar, didSelect date: Date?) {
        ...
        dDay = weekNumber as String
        dMonth = monthName as String
        dWeek = weekDay as String
        
        let datesToSend = Dates(day: dDay, weekday: dWeek, month: dMonth, fullDate: apiDate)
        self.delegate?.acceptData(datesToSend)
    }
